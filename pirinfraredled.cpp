...
bool PIRInfraredLED::openLircDevice()
{
  ...
  if (fileDescriptor == -1)
  {
    std::string errStr = "Failed to connect to ";
  ...
  int space)
{
  if (index >= (BUFFER_SIZE - 1))
  {
    ...
    std::cout << "Buffer overflow in PIRCommandBuffer object." << std::endl;
    return false;
  }
...
//    emit errorMessage("Empty command string.");
    std::cout << "Empty command string." << std::endl;
...
//    errStr += strerror(errno);
//    emit errorMessage(errStr);
     std::cout << "IR device returned error: " << std::endl;
    return false;
  }
#endif
